...
public class LoginController {
    ...
    R login(@Valid @RequestBody LoginDTO loginDTO, HttpServletRequest request, HttpServletResponse response) {
        ...
        List<String> rolename=roleService.RoleNamesByUserId(userDO.getUserId());
        String rolenames = String.join(",",rolename);
        UserToken userToken = new UserToken(userDO.getUsername(), userDO.getUserId().toString(), userDO.getName(),rolenames);
        String token="";
        try {
